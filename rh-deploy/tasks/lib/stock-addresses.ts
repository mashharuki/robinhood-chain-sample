/**
 * Robinhood Chain メインネット (chainId 4663) の実 Stock Token アドレス表。
 * 出典: .claude/skills/robinhood-chain/references/stock-tokens-and-oracles.md
 *
 * 注意:
 * - これらは read-only で照会するだけ。実 Stock Token はアプリからミントできない。
 * - Chainlink フィードのアドレスはここに載せない（フィードは Chainlink が管理して
 *   おりハードコード禁止 — 必要なら stock:live の --feed オプションで渡す）。
 * - テストネットには Stock Token が存在しない。
 */
export const MAINNET_STOCK_TOKENS: Record<string, `0x${string}`> = {
  // 株式
